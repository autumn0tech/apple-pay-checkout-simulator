...
export default function ApplePaySheet({
  ...
  useEffect(() => {
    if (step === "validating") {
      onStepChange(isRecurring ? "recurringValidateMerchant" : "validateMerchant");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:w-[400px] bg-[#1c1c1e] rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-300">
        {/* Header */}
        <div className="bg-[#2c2c2e] px-5 py-4 flex items-center justify-between border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-semibold text-base">{isRecurring ? "Subscribe" : "Pay"}</span>
            {isRecurring && (
        ...
        {step === "validating" && (
          <div className="px-5 py-10 text-center">
            <div className="w-12 h-12 mx-auto mb-4 relative">
              <div className="w-12 h-12 rounded-full border-2 border-blue-400/30 border-t-blue-400 animate-spin absolute inset-0"></div>
            </div>
        ...
        {step === "selecting" && (
          <div className="px-5 py-5 space-y-3">
            ...
            {isRecurring && recurringDetails && (
              <div className="bg-[#2c2c2e] rounded-2xl p-4 space-y-3">
                ...
                <div className="text-[10px] text-gray-500 leading-relaxed px-1">
                  By subscribing, you authorize Apple Demo Store to charge $0.00 for 1 month, then ${recurringDetails.regularAmount}/{recurringDetails.interval} until you cancel. You can manage or cancel at{" "}
                  <span className="text-blue-400 underline cursor-pointer">yourdomain.com/subscriptions</span>. No refunds for partial periods.
                </div>
              </div>
            )}
            ...
            >
              ...
              {isRecurring ? "Double Click to Subscribe" : "Double Click to Pay"}
            </button>
        ...
        {step === "authorizing" && (
          <div className="px-5 py-10 text-center">
            <div className="relative w-16 h-16 mx-auto mb-4">
              <svg className="w-16 h-16 -rotate-90" viewBox="0 0 64 64">
                ...
                <circle
                  cx="32" cy="32" r="28"
                  fill="none"
                  stroke={isRecurring ? "#a78bfa" : "#34d399"}
                  strokeWidth="4"
                  strokeLinecap="round"
            ...
            <p className="text-white font-medium text-sm">
              {isRecurring ? "Creating billing agreement..." : "Authorizing payment..."}
            </p>
            <p className="text-gray-400 text-xs mt-1">
              {isRecurring ? "Storing recurring token securely" : "Processing with issuing bank"}
            </p>
          </div>
        )}
        ...
        {step === "done" && (
          <div className="px-5 py-10 text-center">
            <div className={`w-16 h-16 ${isRecurring ? "bg-purple-500" : "bg-green-500"} rounded-full flex items-center justify-center mx-auto mb-4`}>
